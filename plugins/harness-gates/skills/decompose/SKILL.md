...
2. **Split toward the atomic grammar** (`https://github.com/ejclark/dungeon-crawler/blob/main/plugins/harness-core/docs/COACHES.md`): **atoms** (one job, no siblings' knowledge) →
...
- **`authenticator.ts` caveat:** its inline login JS is a TS template literal — no backticks/`${}` inside
  it. Extracting there means real `.ts` modules + a re-inline build step (`https://github.com/ejclark/dungeon-crawler/blob/main/plugins/harness-core/docs/ENGINEERING.md`), not
  string juggling.
