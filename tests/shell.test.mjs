...
const launchers = () => {
  ...
  for (const p of readdirSync(PLUGINS, { withFileTypes: true })) {
    ...
    const binDir = join(PLUGINS, p.name, "bin");
    try {
      for (const f of readdirSync(binDir)) out.push(join(binDir, f));
    } catch {
      /* a plugin need not ship launchers */
    }
  }
  return out;
};
...
test("every shipped launcher passes shellcheck", () => {
  const files = launchers();
  assert.ok(files.length > 0, "no launchers found — the glob is wrong, not the repo");
  if (!available()) {
